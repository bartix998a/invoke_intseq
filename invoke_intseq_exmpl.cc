...
template <class T, T... t> struct Printer {
    static void Print() {
        ((std::cout << t << ' '), ...);
        std::cout << std::endl;
    }
...
struct Foo {
    ...
    Foo(const Foo &o) {
        Init();
        std::cout << "Foo(const Foo&) id = " << o.id << " -> id = " << id
                  << std::endl;
    }
    ...
    void Print() { std::cout << "Foo::Print() id = " << id << std::endl; }
};
...
int main() {
    ...
    static_assert(
        std::is_same_v<decltype(invoke_intseq([](auto...) {}, 1, 2, 3)), void>);
    static_assert(
        std::is_same_v<
            decltype(invoke_intseq([](auto...) { return 0; }, 1, 2, 3)), int>);
    static_assert(
        std::is_same_v<decltype(invoke_intseq(
                           [](auto...) {},
    ...
    invoke_intseq([](auto... a) { Print(a...); },
                  std::integer_sequence<int, 1, 2, 3>(), size_t(10),
    ...
    invoke_intseq([](auto... a) { Print(a...); }, "ala", size_t(10),
                  std::integer_sequence<int, 7, 8>());
    ...
    invoke_intseq([](const auto &...a) { Print(a...); },
                  std::make_index_sequence<2>(), Foo(), foo);
